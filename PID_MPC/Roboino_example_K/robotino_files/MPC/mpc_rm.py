...
class mpc_rm:
    def __init__(self):
        ...
        # self.N=self.sym_r.N
        self.opts={'ipopt.linear_solver':'ma57','ipopt.warm_start_init_point':'yes',"ipopt.max_iter": 15000 }
    ...
    # # build the problem in here #
        for k in range(self.sym_m.N-1):
            ...
            g1          +=[-xkr1+self.R_model.runga_kutta(xkr,omega[:,k],self.sym_r.Ts)]
            x           +=[xkr1]
            ...
            u           +=[ um[:,k+1] ]
            g3        +=[ -um[:,k+1] + self.motor_model.Au_modify(K11,K22,K33,e_k[:,k],um[:,k]) ]
            xkr=xkr1
            e0= xkr1-xr_ref_desired[:,k+1]
            J= J + e0.T @ self.sym_m.Q @ e0
            
            
        self.z=x+eta_acc+u+[veccat(rm)]+[vertcat(K1)]+[vertcat(K2)]+[vertcat(K3)]      
        self.g=g1+g2+g3
