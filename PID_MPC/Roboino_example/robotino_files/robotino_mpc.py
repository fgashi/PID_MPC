...
zv=np.zeros(((rob.n_states_m+rob.n_states_r+rob.sym_m.n_inputs)*(rob.sym_m.N-1)+(rob.sym_m.n_motors*rob.sym_m.N),1))
...
references=am[(N-1)*9+3*(N-1)::]

ref_0=references[0::3]
ref_1=references[1::3]
ref_2=references[2::3]

ref_optimal=np.hstack((np.matrix((ref_0)).T,np.matrix((ref_1)).T,np.matrix((ref_2)).T))
...
total_results=np.hstack((position_res,states_res,inputs_res,ref_optimal,np.matrix((tgrid)).T))
...
if T==True:
    ...
    reference_gen='is_not_constant'
    ref_amplitude=amplitude
    ref_frequency=0*frequency


sol_setup=np.hstack(( np.matrix((t2_wall_sol)),t1_wall,np.matrix((ref_amplitude)),np.matrix((ref_frequency)),np.matrix((rob.sym_m.Q[0,0])),np.matrix((rob.sym_m.Q[1,1])),np.matrix((rob.sym_m.Q[2,2])), np.matrix((rob.motor_model.Kp)), np.matrix((rob.motor_model.Ki)), np.matrix((rob.motor_model.Kd)) ))


# np.savetxt('/home/fatos/python_ws/git_files/overleaf_files/Graphics/Robotino_example/ref_k/results_N_'+str(N)+'.txt',total_results, delimiter='  ',header='   x1      x2      x3      i1      i2      i3      omega_1      omega_2      omega_3      input_1      input_2      input_3      ref_1      ref_2      ref_3      t' ,comments=' ')
# np.savetxt('/home/fatos/python_ws/git_files/overleaf_files/Graphics/Robotino_example/ref_k/ref_N_'+str(N)+'.txt',reference, delimiter='  ',header='  x1_d    x2_d    x3_d      t' ,comments='')
# np.savetxt('/home/fatos/python_ws/git_files/overleaf_files/Graphics/Robotino_example/ref_k/sol_setup_N_'+str(N)+'.txt',sol_setup, delimiter='  ',header='  t_wall_warm_start    t_wall_non_warm_start    ref_amplitude    ref_frequency    Q11    Q12   Q13     Kp      Ki      Kd'    ,comments='')
